...
function StatItem({ value, label, highlight }: StatItemProps) {
  return (
    <div className="flex flex-col gap-0.5">
      <p className={`text-2xl font-bold tabular-nums leading-none ${highlight ? "text-primary" : ""}`}>
      ...
      <Card className="flex flex-col">
        <CardContent className="p-5 flex flex-col flex-1 gap-5">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
            <StatItem value={visitsToday} label="Today" highlight />
            <StatItem value={visitsThisMonth} label="This Month" />
            <StatItem value={totalVisits} label="Total Visits" />
            <StatItem value={uniqueVisitors} label="Unique Visitors" />
          </div>
          ...
          {formatted.length > 0 && (
            <>
              ...
              <div className="flex flex-col gap-2">
                ...
                <ChartContainer config={dailyConfig} className="h-[180px] w-full">
                  <AreaChart data={formatted} margin={{ top: 4, right: 4, left: -16, bottom: 0 }}>
                    <defs>
                      <linearGradient id="gv" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="var(--color-visits)" stopOpacity={0.25} />
                        <stop offset="95%" stopColor="var(--color-visits)" stopOpacity={0} />
                      </linearGradient>
                      <linearGradient id="gu" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="var(--color-unique)" stopOpacity={0.15} />
                ...
                <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                  {truncatedPages.map((_, i) => (
                    <Cell
                      ...
                      fillOpacity={Math.max(1 - i * 0.08, 0.3)}
                    />
                  ))}
                </Bar>
              </BarChart>
